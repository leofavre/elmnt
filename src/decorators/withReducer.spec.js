...
describe('withReducer', () => {
  ...
  describe('#dispatchAction()', () => {
    ...
    it('Should return this.state if an object is passed as an action.', () => {
      const dummy = new Dummy();
      const newState = dummy.dispatchAction({ type: 'UPGRADE', level: 5 });
      expect(newState.level).to.equal(5);
    });
    ...
    it('Should change this.state if an asynchronous function ' +
      ...
      expect(asyncDispatch).to.be.a('promise');

      await asyncDispatch;
      expect(dummy.state.level).to.equal(5);
    });

    it('Should return this.state if an asynchronous function ' +
      'is passed as an action.', async () => {
      const dummy = new Dummy();
      const asyncUpgrade = Promise.resolve({ type: 'UPGRADE', level: 5 });
      const asyncDispatch = dummy.dispatchAction(asyncUpgrade);
      ...
      const newState = await asyncDispatch;
      expect(newState.level).to.equal(5);
    });
    ...
    it('Should return this.state if a thunk receiving dispatch and ' +
      'getState is passed as an action.', () => {
      const dummy = new Dummy();
      dummy.dispatchAction({ type: 'UPGRADE', level: 5 });
      ...
      const newState = dummy.dispatchAction(thunkAction);

      expect(newState.level).to.equal(10);
    });
  });
});
